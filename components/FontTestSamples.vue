<template>
  <div class="text-white">
    <div class="flex justify-center items-center h-screen w-full">
      <div class="flex flex-wrap flex-col justify-center w-1/2">
        <Logo class="mx-auto my-4" size="small" />
        <Logo class="mx-auto my-4" size="medium" />
        <Logo class="mx-auto my-4" size="large" />
      </div>

      <div class="flex justify-center w-1/2">
        <h4 class="font-pollux route--underline">Pollux fonts samples</h4>
      </div>
    </div>

    <div class="container flex flex-wrap items-center mx-auto py-12">
      <div class="w-full">
        <div class="text-right font-extrabold" data-aos="zoom-out-up">
          <h2>Pollux</h2>

          <p>
            Scale fast your business with expert software engineering teams.
          </p>
        </div>

        <div class="text-right font-title" data-aos="zoom-out-up">
          <h2>Pollux</h2>

          <p>
            Scale fast your business with expert software engineering teams.
          </p>
        </div>

        <div class="text-right font-mono" data-aos="zoom-in-up">
          <h2>Pollux</h2>

          <p>
            Scale fast your business with expert software engineering teams.
          </p>
        </div>

        <div class="text-right font-pollux" data-aos="zoom-in-up">
          <h2>Pollux</h2>

          <p>
            Scale fast your business with expert software engineering teams.
          </p>
        </div>
      </div>

      <div class="w-full flex flex-wrap flex-row justify-end">
        <div class="p-6 text-white">
          <a target="_blank" href="https://icons8.com/icon/HKUjzhHqqOJK/client">
            Client
          </a>
          icon by <a target="_blank" href="https://icons8.com">Icons8</a>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {}
</script>

<style lang="scss" scoped>
h2 {
  font-size: 132px;
}

h4 {
  font-size: 50px;
  margin-bottom: 1.5rem;
}

p {
  font-size: 28px;
  margin-bottom: 1rem;
}
</style>
